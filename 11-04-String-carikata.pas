...
Procedure CariKata(x:Integer;Carikt:String;Var indeks:String;Var ktm:Integer);
...
While i<=Length(Kal) Do
 Begin
  If (i=1) And (Kal[1]<>' ') Then
   AmbilKata(i,j,Kal)
  Else If (Kal[i]=' ') And (Kal[i+1]<>' ') And (Kal[i+2]<>' ') Then
   Begin
    Inc(i);
    AmbilKata(i,j,Kal);
   End
  Else If (Kal[i]='-') And (Kal[i-1]<>' ') And (Kal[i+1]<>' ') Then
   Begin
    Inc(i);
    AmbilKata(i,j,Kal);
   End
